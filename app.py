...
with col_title:
    ...
    st.markdown("<p style='font-size: 1.2rem; color: #666; margin-top: 5px;'>Official Campus Marketplace, Asset Rental & Student Charity Fund Portal</p>", unsafe_allow_html=True)
...
# Tabs
tab1, tab2, tab3, tab4 = st.tabs(["🛍️ Browse InstiMart", "➕ Post New Ad (Login Required)", "🤝 Settle Deal", "🎗️ Campus Charity Ledger"])
...
with tab1:
    ...
    df_feed.columns = [
        "Item ID", "Title", "Category", "Price (₹)", "Listing Type", 
        "Rental Rate (₹/day)", "Condition", "Seller Name", 
...
with tab2:
    ...
    if not st.session_state.logged_in:
        ...
        with st.form("new_listing_form"):
            ...
            with col2:
                price = st.number_input("Selling Price / Deposit (₹)", min_value=0.0, step=100.0, value=500.0)
    ...
    if not st.session_state.logged_in:
        ...
        with st.form("transaction_form"):
            ...
            if transact_btn:
                try:
                    cursor = conn.cursor()
                    # Trigger calculates 2% charity automatically
                    cursor.execute('''
...
with tab4:
    ...
    # Calculate Total Charity Collected
    cursor = conn.cursor()
